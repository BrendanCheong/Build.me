...
const Modal = ({ modalClose, ChartDataLoading, LineChartData, itemName, name, id}) => {
    ...
    return (
        <div>
            <dialog id={`${name}Modal${id}`} className="relative z-0 w-screen h-screen bg-transparent">
                    ...
                    <div className="relative flex w-8/12 h-full bg-white rounded-lg">
                        <div className="flex flex-col items-start w-full">
                            <div className="flex items-center w-full text-center p-7">
                                ...
                                <svg onClick={() => modalClose(`${name}Modal${id}`)} className="w-8 h-8 ml-auto transition duration-300 rounded-full cursor-pointer text-cool-gray-800 hover:text-purple-600 focus:text-purple-600 hover:bg-red-500" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 18 18">
                            ...
                            <div className="flex items-center justify-end w-full p-7">
                                <button type="button" onClick={() => modalClose(`${name}Modal${id}`)} className="px-4 py-2 font-semibold text-teal-700 duration-300 bg-transparent border border-teal-500 shadow-md rounded-xl hover:bg-indigo-500 hover:text-white hover:border-transparent focus:outline-none">
                                    Close
                                </button>
                            </div>
                        </div>
