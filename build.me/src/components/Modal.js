...
const Modal = ({ open, onClose, animated, animation, info, setInfoState, id, card, name}) => {

    if(!open) return null // if not open, render nothing, else render below box
                ...
                {/* <animated.div style={animation}> */}
                    ...
                    {/** <!-- Header Start --> */}
                        <div className="flex items-center justify-center w-full h-auto space-x-36">
                                <div className="flex items-center justify-center w-10/12 h-auto py-3 pl-56 text-2xl font-bold font-poppins">
                                    {"Choose A Seller! " + info }
                                </div>

                                <div onClick={onClose} className="flex justify-center object-right-top h-auto p-2 duration-300 rounded-full cursor-pointer hover:bg-red-500">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#000000" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-x"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line>
                                    </svg>
                                </div>
                    {/* <!--Header End--> */}
                        </div>
                        ...
                        <div className="flex flex-col space-y-12">
                            ...
                            <div>
                                ...
                                <div className="flex items-start px-8 pb-8 space-x-4 overflow-x-auto overflow-y-scroll text-center bg-white rounded scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-200 scrollbar-thumb-rounded scrollbar-track-rounded hover:scrollbar-thumb-gray-500">
                                        <ModalCard id={id} card={card} name={name} info={info}/>
                                        <ModalCard id={id} card={card} name={name} info={info}/>
                                        <ModalCard id={id} card={card} name={name} info={info}/>
                                        <ModalCard id={id} card={card} name={name} info={info}/>
                                        <ModalCard id={id} card={card} name={name} info={info}/>   
